    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: 'assistant',
          content: 'Got it! Integrating your request and modifying the code below. Building preview...',
  ...
  return (
    <div className="flex w-full h-full bg-[#0a0a0a] text-gray-200">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        ...
        {/* Workspace Grid */}
        <div className={`flex-1 min-h-0 bg-[#050505] ${activeTab === 'split' ? 'grid grid-cols-2' : 'flex flex-col'}`}>
          {(activeTab === 'split' || activeTab === 'code') && (
            <div className={`relative min-h-0 flex flex-col ${activeTab === 'split' ? 'border-r border-zinc-800' : 'flex-1'}`}>
              <CodeEditor code={code} onChange={(v) => setCode(v || '')} />
            </div>
          )}
          
          {(activeTab === 'split' || activeTab === 'preview') && (
            <div className={`relative min-h-0 flex flex-col bg-[#111] ${activeTab === 'preview' ? 'flex-1' : ''}`}>
              <Preview />
            </div>
          )}
        </div>
