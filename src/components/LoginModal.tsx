    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] animate-in fade-in duration-200">
      <div className="bg-[#0f0f0f] border border-zinc-800 rounded-xl w-full max-w-sm shadow-2xl flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleLogin} className="p-6 flex flex-col gap-4">
          ...
          {error && <div className="text-[10px] text-red-500 font-mono text-center uppercase tracking-widest bg-red-500/10 py-1 rounded">{error}</div>}

          <div>
            <label className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest mb-1 block">Email</label>
            <input 
              type="email" 
              value={email}
          ...
          <div>
            <label className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest mb-1 block">Password</label>
            <input 
              type="password" 
              value={password}
          ...
          <button type="submit" className="mt-4 bg-orange-600 hover:bg-orange-500 text-black w-full py-2.5 rounded text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2">
