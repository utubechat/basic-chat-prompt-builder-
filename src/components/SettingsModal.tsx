...
export function SettingsModal({ onClose, accentColor, onColorChange }: SettingsModalProps) {
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] animate-in fade-in duration-200">
      <div className="bg-[#0f0f0f] border border-zinc-800 rounded-xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-6">
          <div>
            <h3 className="text-[11px] font-bold text-zinc-500 tracking-widest uppercase mb-3 text-left">Accent Color</h3>
            <div className="grid grid-cols-3 gap-3">
              {COLORS.map((color) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-zinc-400 font-mono">{color.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end">
             <button onClick={onClose} className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-4 py-2 rounded text-xs font-bold uppercase tracking-widest transition-colors">
               Done
             </button>
          </div>
        </div>
